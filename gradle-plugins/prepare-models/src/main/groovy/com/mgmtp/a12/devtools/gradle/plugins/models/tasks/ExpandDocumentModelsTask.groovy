...
@CacheableTask
abstract class ExpandDocumentModelsTask extends ModelConversionTask {
    ExpandDocumentModelsTask() {
        mainClass.set('com.mgmtp.a12.kernel.md.facade.cli.BatchDocumentModelExpander')
        group = 'prepare-models-plugin'
        description = 'Expands the document models in the configured output directory.'
    ...
    @TaskAction
    void expand(InputChanges inputChanges) {
        args inputDir.get().asFile.getAbsolutePath()

        handleChanges(inputChanges, "json")
    }
}
